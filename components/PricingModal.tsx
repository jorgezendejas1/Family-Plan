...
import { X, Check, Zap, Smartphone, Cpu, ShieldCheck, MessageSquare } from 'lucide-react';
  ...
  const plans = [
    {
      ...
      features: ['Uso de App Libre', '1 Miembro Familia', 'Chat Familiar FIFO 200', 'Cifrado E2EE', '1 cuenta Google', 'Con Anuncios'],
    ...
    },
    {
      ...
      subtitle: 'Parejas y familias pequeñas.',
      features: ['Todo lo del plan Gratis', 'Hasta 20 Miembros', 'Multi-Sincronización Google', 'Prioridad IA', 'Sin anuncios', 'Soporte 24/7'],
    ...
    },
    {
      ...
      subtitle: 'Uso intensivo.',
      features: ['Todo lo del plan Basic', 'Capacidad IA Extendida', 'Análisis Fotos/PDFs', 'Roles Avanzados', 'Exportación Pro'],
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in">
      <div className="bg-white dark:bg-black rounded-[40px] shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] border border-white/20 dark:border-zinc-800">
        <div className="p-6 flex items-center justify-between border-b border-gray-100 dark:border-zinc-800">
          <div className="flex flex-col">
            <h2 className="text-2xl font-bold dark:text-white flex items-center gap-2">
                <Cpu className="text-blue-500" size={24} /> Potencia de Agendamiento IA
            </h2>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Límites de creación de eventos por Inteligencia Artificial</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 grid md:grid-cols-3 gap-6 custom-scrollbar">
          {plans.map((plan, idx) => (
            <div key={idx} className={`relative p-8 rounded-[32px] border transition-all ${plan.color} flex flex-col hover:scale-[1.02] shadow-xl`}>
              {plan.highlight && <div className="absolute top-4 right-4 bg-blue-500 text-white text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter">RECOMENDADO</div>}
              ...
              <div className="mb-8 px-4 py-2 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 w-fit flex flex-col items-center">
                <span className="text-[11px] font-black text-blue-600 dark:text-blue-400 tracking-wider uppercase">{plan.limit}</span>
                <span className="text-[9px] opacity-70 font-bold dark:text-gray-400">{plan.monthlyLabel}</span>
              </div>
              ...
              <ul className="space-y-3 mb-10 flex-1 pt-2">
                {plan.features.map((f, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-xs text-gray-600 dark:text-gray-300 font-bold leading-tight">
                    <div className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${plan.highlight ? 'bg-blue-500 text-white' : 'bg-green-500/20 text-green-600'}`}>
