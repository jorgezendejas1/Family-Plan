...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const aiLimitDisplay = useMemo(() => {
    const p = currentUser.plan;
    if (p === 'unlimited' || p === 'admin' || p === 'casa') return "ILIMITADO";
    if (p === 'pro') return "125 / SEM\n(500 / MES)";
    if (p === 'basic') return "50 / SEM\n(200 / MES)";
    return "10 / SEM\n(40 / MES)";
  }, [currentUser.plan]);
  ...
  const handleUpdateCalendar = async (id: string, updates: Partial<CalendarConfig>) => {
    if (id === 'new') {
        ...
        onCalendarsChange(calendars.map(c => c.id === id ? { ...c, ...updates } : c));
    }
  ...
  const handleResetToDefaults = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (window.confirm('¿Restablecer calendarios y borrar eventos?')) {
        setIsResetting(true);
        try {
  ...
  return (
    <>
        ...
        <div className="h-full overflow-y-auto custom-scrollbar flex flex-col p-6">
          ...
          <div className="mb-8 shrink-0">
            <div className="flex items-center justify-between mb-3 px-1">
                ...
                <div className="flex items-center gap-1">
                    {(currentUser.plan === 'admin' || currentUser.plan === 'casa') && (
                        <button 
                            disabled={isResetting}
                            onClick={handleResetToDefaults}
                            className={`p-2 text-gray-400 hover:text-orange-500 rounded-lg transition-all ${isResetting ? 'animate-spin' : ''}`}
                        >
                            <RotateCcw size={16} />
                        </button>
                    )}
                    {canAddMore && (
                        <button onClick={() => setEditingCalendar({ id: 'new', label: '', color: EVENT_COLORS[0], visible: true })} className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all">
          ...
          <div className="mt-4 pt-6 border-t border-gray-100 dark:border-zinc-800 flex flex-col gap-4 shrink-0 pb-4">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-xl w-fit">
                   ...
                   <span className="text-[9px] font-black text-green-600 dark:text-green-400 uppercase tracking-widest flex items-center gap-1"><ShieldCheck size={10} /> E2EE ACTIVO</span>
              </div>
                
              <div onClick={onOpenPricing} className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl px-4 py-3 text-white shadow-lg hover:scale-[1.02] active:scale-95 transition-all cursor-pointer flex items-center justify-between">
                <div>
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80 block">PLAN ACTUAL</span>
                  <p className="text-sm font-black uppercase">{currentUser.plan}</p>
                </div>
                <div className="text-right">
                  <span className="text-[8px] opacity-70 block">POTENCIA IA</span>
                  <p className="text-[9px] font-bold leading-tight whitespace-pre-line">{aiLimitDisplay}</p>
                </div>
              </div>
      ...
      <CalendarEditModal 
        isOpen={!!editingCalendar} onClose={() => setEditingCalendar(null)} 
        calendar={editingCalendar} onSave={handleUpdateCalendar} 
        onDelete={handleDeleteCalendar} canDelete={calendars.length > 1}
      />
    </>
  );
};
